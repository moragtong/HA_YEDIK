...
namespace p2p_client {
	struct p2p_client {
		...
		void request_list() {
			command_tracker(LIST);
			size_t size;
			socket::recvfrom(recv_sock, (char*)&size, sizeof(size), 0, 0, 0);
			std::cout << size;
			socket::recvfrom(recv_sock, (char*)clients.begin(), size * sizeof(util::sockaddr), 0, 0, 0);
			clients._end = clients._buff + size;
